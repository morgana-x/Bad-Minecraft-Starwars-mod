package morgana.newmobs.entity.ai.goal;
...
public class BlasterAttackGoal<T extends HumanEntity> extends Goal {
    ...
    public BlasterAttackGoal(T actor, double speed, int attackInterval, float range) {
        this.actor = actor;
        this.speed = speed;
    ...
    protected boolean isHoldingBow() {
        return this.actor.isHolding(BLASTER_BASIC); //  isHolding(BLASTER_BASIC);
    }
        ...
        if (livingEntity != null) {
            ...
            if (this.actor.isUsingItem()) {
                if (!bl && this.targetSeeingTicker < -60) {
                    ...
                    if (i >= 20) {
                        this.actor.clearActiveItem();
                        ((RangedAttackMob)this.actor).shootAt(livingEntity, 1); //, BowItem.getPullProgress(i));
                        this.cooldown = this.attackInterval;
                    }
                }
            } else if (--this.cooldown <= 0 && this.targetSeeingTicker >= -60) {
                this.actor.setCurrentHand(ProjectileUtil.getHandPossiblyHolding(this.actor, BLASTER_BASIC ));
            }
